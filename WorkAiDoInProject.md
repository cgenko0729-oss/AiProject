...
## 追加作業 — BSP ダンジョン生成（提出後のユーザー要望）

日付: 2026-07-04（本編完了後）
ブランチ: feature/bsp-dungeon
要望: 「マップが狭い。`Assets/ScreenShotForAi/GameDungeonImagePhoto.png`
（風来のシレン系ローグライクのミニマップ＝矩形部屋＋細い通路）のような
ダンジョンを BSP で生成してほしい」

### 手順

1. **参考画像の読解** — スクリーンショットのミニマップから
   「複数の矩形部屋を1マス幅の通路でつなぐ」古典ローグライク構造と特定
2. **GameConfig 変更**
   - `width` 10→40, `height` 10→24, `max_turn` 100→300, `enemy_count` 3→8
   - `wall_percent`（確率壁方式の遺物）を削除
   - `min_area_size = 8`（BSP 分割の最小エリア）, `min_room_size = 4` を追加
3. **RogueGame にBSP生成を実装**（すべて private、新規公開クラスなし）
   - 内部専用 `readonly struct Room`（x, y, w, h, center）を追加
   - `_CreateMap(rooms)`: 全マスを壁で埋める → `_SplitArea` 再帰分割 →
     生成順に `_CarveCorridor` で鎖状接続（全部屋連結を保証）
   - `_SplitArea`: 長い軸を優先して乱数位置で2分割（規則固定＝決定論維持）。
     `min_area_size * 2` 未満になったら `_CarveRoom` で部屋化
   - `_CarveRoom`: エリア境界1マスを残し、乱数サイズ・乱数位置の部屋を掘る
   - `_CarveCorridor`: 部屋中心間を横→縦のL字で掘る
   - `_PickGoal`: スポーンからマンハッタン距離最大の部屋の中心をゴールに。
     部屋が1つだけの小マップでは部屋の隅、置けなければ例外（握り潰さない）
   - プレイヤーのスポーンは固定 (1,1) から「最初の部屋の中心」へ変更
4. **テスト更新**（7本とも維持・Assert は弱めない）
   - スポーン固定座標に依存していた 4 テストを座標非依存の書き方へ:
     壁テスト = 左へ歩き続けて必ず壁で止まる方式 /
     攻撃テスト = `player.pos.Next(Right)` に敵を置く方式 /
     ゴールテスト = 6x6（部屋1つ）でゴールをスキャンして直進
   - `dotnet test` → **7 passed / 0 failed**
5. **ASCII マップダンプによる目視検証**
   - スクラッチ用コンソールで Seed 12345 / 1 / 777 の 3 枚を出力
   - 「複数矩形部屋 + 1マス通路 + P が始点部屋・G が遠方部屋」を確認
6. **RogueGameView 改良**
   - 床に接しない岩盤内部の壁キューブを描画しない `_HasFloorNeighbor` を追加
   （見た目が参考画像に近づき、オブジェクト数も削減。表示のみの変更）
7. **検証** — check-core.ps1 PASS / Core・View とも dotnet で 0 エラー
8. **ドキュメント更新** — core-dev SKILL のマップ生成手順、FINAL_REVIEW の
   既知の制限（「ゴール到達保証なし」→ BSP で保証に変更）、PLAN.md 追記

### この変更で直った既知の制限

旧方式の「Seed によってはゴールが壁で塞がれ詰む」問題は、
BSP の鎖状通路接続により**全部屋連結が構造的に保証される**ため解消。
